...
st.markdown(f"""
<style>
    /* Center the main header and sub-header */
    .header-container {{
        text-align: center;
        margin-bottom: 20px;
    }}
    ...
    /* Change "Drop skin image here" (Widget Labels) */
    div[data-testid="stWidgetLabel"] p {{
        color: grey !important;
        font-weight: bold !important;
    }}

    /* Change "Original Image" & "AI Detections" (Image Captions) */
    .stImageCaption {{
        color: #1bbfb4 !important;
        ...
        try:
            ...
            with col_result:
                ...
                if result.get("annotated_image"):
                    ...
                    st.image(img_data, caption="AI Detections", use_container_width=True)
                ...
                # Wrapped in a div with the custom severity-text-header class
                st.markdown(f"""
                ...
                st.markdown(f"""
                    <div class="severity-container">
                        <div class="severity-bar" style="width: {severity_score}%; background-color: {bar_color};">
                            {severity_score}%
                        </div>
                    </div>
                """, unsafe_allow_html=True)
            ...
            with bottom_col1:
                ...
                if conditions:
                    for condition in conditions:
                        clean_name = condition.replace('_', ' ').title()
                        st.markdown(f'<div class="condition-badge">{clean_name}</div>', unsafe_allow_html=True)
                else:
                    st.info("No clinical issues detected.")

            with bottom_col2:
                st.subheader("📋 Clinical Recommendation")
                recommendation = result.get("recommendation", "No specific recommendation available.")
                st.markdown(f"""
...
# --- Footer ---
st.markdown("<br><br><p style='text-align: center; color: #aaa;'>Developed with ❤️ for a healthier skin journey.</p>", unsafe_allow_html=True)